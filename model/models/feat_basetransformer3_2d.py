...
class FEATBaseTransformer3_2d(FEATBaseTransformer3):
    ...
    def _forward(self, instance_embs, support_idx, query_idx, ids=None, simclr_embs=None, key_cls=None, return_intermediate=False
        ...
        if self.args.method == 'MBT' or self.args.method == 'PMBT':
            base_protos = self.memory[torch.Tensor(top_indices).long()].reshape(5, 5, 64, 5, 5)
        elif self.args.method == 'KVMBT':
            base_protos = self.memory[torch.Tensor(top_indices).long()].reshape(5, 5, 64, 5, 5)
            base_protos_target = self.memory_target[torch.Tensor(top_indices).long()].reshape(5, 5, 64, 5, 5)
            ...
            base_protos_target = base_protos_target.reshape(b1*b2, b3*b4*b4)

            proto = apply_z_norm(proto)
            base_protos = apply_z_norm(base_protos)
            base_protos_target = apply_z_norm(base_protos_target)
            ...
            base_protos_target = base_protos_target.permute(0, 2, 1, 3, 4).contiguous()
            combined_protos_target = base_protos_target.reshape(n_class*n_batch, emb_dim, -1).permute(0, 2, 1).contiguous()
            proto = self.slf_attn(proto, combined_protos, combined_protos_target)
        ...
        if self.args.use_euclidean:
            if self.args.method == 'KVMBT':
                proto_target = instance_embs_target[:5].view(5, 1600)
                proto_target = apply_z_norm(proto_target)
                logits_kvm = - torch.mean((proto.view(1, 5, 1600) - proto_target.view(5, 1, 1600)) ** 2, 2) / self.args.temperature
            query = query.view(-1, emb_dim).unsqueeze(1) # (Nbatch*Nq*Nw, 1, d)
            proto = proto.unsqueeze(1).expand(num_batch, num_query, num_proto, emb_dim).contiguous()
            proto = proto.view(num_batch*num_query, num_proto, emb_dim) # (Nbatch x Nq, Nk, d)
