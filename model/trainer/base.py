...
class Trainer(object, metaclass=abc.ABCMeta):
    def __init__(self, args):
        ...
        self.logger = Logger(args, osp.join(args.save_path), wandb_run=self.wandb_run)
    ...
    def try_evaluate(self, epoch):
        ...
        if self.train_epoch % args.eval_interval == 0:
            ...
            self.trlog['final_val_acc'] = va
            self.trlog['final_val_acc_interval'] = vap

    def try_logging(self, tl1, tl2, ta, aux_loss=None, tg=None):
        args = self.args

        self.logger.add_scalar('train_total_loss', tl1.item(), self.train_step)
        self.logger.add_scalar('train_loss_meta', tl2.item(), self.train_step)
        # if aux_loss is not None:
        #     self.logger.add_scalar('train_aux_loss', aux_loss.item(), self.train_step)
        self.logger.add_scalar('train_acc',  ta, self.train_step)
